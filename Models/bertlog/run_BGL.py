...
# 记录日志
filename = "../../OutPut/BERTLog/run_BGL_by%%"
...
class BertClassificationModel(nn.Module):
    def __init__(self):
        ...
        model_class, tokenizer_class, pretrained_weights = (tfs.BertForSequenceClassification, tfs.BertTokenizer, 'bert-base-uncased')
...
'''
    1.加载数据集
'''
train_sets = pd.read_csv('../../Dataset/BGL/BGL_sequence.csv', header=0)  # 训练集

# 数据集大小分割，分别取10%，20%，50%
train_sets = train_sets[0: int(len(train_sets) * 1)]

x = train_sets['sequence'].values
y = train_sets['label'].values

# 数据集分配
train_inputs, dev_inputs, train_targets, dev_targets = train_test_split(x, y, test_size=0.99)

print("训练集大小：", len(train_inputs))
print("训练集标签：", len(train_targets))
print("测试集大小：", len(dev_inputs))
print("测试集标签：", len(dev_targets))
