...
class BertClassificationModel(nn.Module):
    def __init__(self):
        ...
        model_class, tokenizer_class, pretrained_weights = (tfs.T5EncoderModel, tfs.T5Tokenizer, 't5-base')
    ...
    def forward(self, batch_sentences):
        ...
        bert_cls_hidden_state = bert_output[0][:, 0, :].cuda()   # 提取[CLS]对应的隐藏状态
        linear_output = self.dense(bert_cls_hidden_state).cuda()
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for j in range(total):
            ...
            for e in sens:
                ...
                cls += e + " "
            cls = "[CLS]" + cls + "[SEB]"
            outputs = bert_classifier_model([cls])  # 此处需要传数组，所以加一个[]
            logits = outputs
    ...
    # result.to_csv('../../../Dataset/HDFS/result_Robert-Log.csv', index=False)
